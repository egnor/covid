...
def get_data(session):
    ...
    sig = numpy.select(
        (numpy.equal(data.status_simp, 'Implementation Suspended'),
         numpy.isin(data.status_simp,
                    ('Strongly Implemented', 'Partially Implemented'))),
        (1, -1),
        default=0)

    sig *= numpy.where(
        numpy.isin(data.intervention_group, (
            'school_closed', 'household_confined', 'office_closed',
            'entertainment_closed', 'store_closed', 'restaurant_closed',
            'state_of_emergency', 'limited_mvt', 'closed_border', 'mask')) &
