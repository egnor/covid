...
argument_group.add_argument('--cache_time', type=pandas.Timedelta,
                            default=pandas.Timedelta(hours=6))
argument_group.add_argument('--cache_dir', type=pathlib.Path,
...
def cached_path(session, url):
    ...
    if isinstance(adapter, cachecontrol.CacheControlAdapter):
        ...
        if (isinstance(cache, cachecontrol.caches.file_cache.FileCache) and
                isinstance(heuristic, _CacheHeuristic)):
